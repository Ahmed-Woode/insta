...
def start(message):
    ...
    maac.row_width = 1
    maac.add(START)
    bot.send_message(message.chat.id,text=f"مرحباً بك في بوت الصيد",parse_mode="markdown",reply_markup=maac)
...
def clase(call):
	if call.data=='START':
		...
		maac2.row_width = 2
		maac2.add(ref)
		bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text=f"* اهلا بكت صيد متاحات انستجرام\n\n@woode_249*",parse_mode="markdown",reply_markup=maac2)
	if call.data=="ref":
		 	cansl = types.InlineKeyboardButton(text =" رجوع ↩️ ", callback_data = 'START')
		 	Keyy = types.InlineKeyboardMarkup()
		 	Keyy.row_width = 1
		 	Keyy.add(cansl)
		 	bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text=f"* ارسل الان كلمة Bot\n\n@woode_249*",parse_mode="markdown",reply_markup=Keyy)
        ...
        "user_id":id
					    					...
					    					msg=f"𝙷𝙸𝚃 : {j}\n𝙽𝙰𝙼𝙴 : {nam}\n𝙴𝙼𝙰𝙸𝙻 : {nn}@gmail.com\n𝙵𝙾𝙻𝙻𝙾𝚆𝙴𝚁𝚂 : {fols}\n𝙵𝙾𝙻𝙻𝙾𝚆𝙸𝙽𝙶 : {fol}\n𝙿𝙾𝚂𝚃 :{post}\n𝙳𝙰𝚃𝙰 : {da}\n𝙸𝙳 :{id}\n𝚁𝙴𝚂𝚃 : {rs}\n𝙻𝙸𝙽𝙺 : https://www.instagram.com/{nn}\n@woode_249 - @woode_249"
					    					...
					    					cc+=1
														...
														aac.row_width = 2
														i12= types.InlineKeyboardButton(text =f"Bad Gmail  : {cc} ", callback_data = 'c')
														i1= types.InlineKeyboardButton(text =f"Bad Instagram : {vv}", callback_data = 'c')
														i11= types.InlineKeyboardButton(text =f"Good Gmail : {h}", callback_data = 'c')
														i7= types.InlineKeyboardButton(text =f"Good Instagram : {us}", callback_data = 'c')
														i8= types.InlineKeyboardButton(text =f"Gmail : {nn}.gmail.com", callback_data = 'c')
														aac.add(i11)
														aac.add(i7)
														aac.add(i12)
														aac.add(i1)
